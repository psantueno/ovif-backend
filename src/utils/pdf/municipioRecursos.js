...
export const buildInformeRecursos = ({ municipioNombre, ejercicio, mes, recursos, totales, usuarioNombre, convenioNombre, cierreId }) => {
  const headerContent = [
    HEADER_BASE64
      ? {
  ...
  if (!Array.isArray(recursos) || recursos.length === 0) {
    ...
    const tableBody = [
      [
        ...
        { text: "PERCIBIDO", style: "tableHeader", alignment: "right" },
      ],
      ...recursos.map((recurso) => [
        { text: recurso.codigo_recurso ?? "-", style: "itemCodigo", alignment: "center" },
        { text: recurso.descripcion ?? "Sin descripción", style: "itemDescripcion" },
        { text: recurso.descripcion_fuente ?? "-", style: "itemDescripcion" },
        { text: formatCurrency(recurso.vigente), alignment: "right", style: "itemImporte" },
        { text: formatCurrency(recurso.percibido), alignment: "right", style: "itemImporte" },
      ]),
      [
        ...
        { text: formatCurrency(totales.percibido), alignment: "right", style: "totalValue" },
      ],
    ];
